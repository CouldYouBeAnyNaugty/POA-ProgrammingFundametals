...
///Union (most comprehensive algorithm, based on set theory)
...
int main(void)
{
	...
	else 
	{
	    for (int i = 0; i<=N; i++)
	    {
	        resultedList[i]=list01[i];
	    }
		for (int i = 1; i<=M; i++)
		{
			int j = 1;
			while (j <= N && list02[i] != list01[j] )
			{
				j++;
			}
			bool exists = (j>N);
			...
			{
				cnt++;
				resultedList[cnt] = list02[i];
			}
			
		}
		///Outputs 
		cout << "The elements in the resulted list are: " << cnt << endl;
	    cout << "Here is the list of Union between the firstList and the secondList: " << endl << "| " ;
	    for (int i=1; i<=cnt; i++)
	    {
		    cout << resultedList[i] << " | ";
	    }
